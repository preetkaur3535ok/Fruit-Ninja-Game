...
'''def draw_white_lives(display,x,y,lives,image):
        
    if player_lives==3:
        whitel=pygame.image.load("white_lives.png")
        redl=pygame.image.load("red_lives.png")
        redl_rect=redl.get_rect()
        redl_rect.x =int(x-35*player_lives)
        redl_rect.y=y
        display.blit(whitel,redl_rect)
...
'''   img = pygame.image.load(image)
        ...
        lives_icon_rect = lives_icon.get_rect()
        lives_icon.x=int(x+35*t)
        lives_icon.y=y
        gameDisplay.blit(lives_icon,lives_icon_rect)
           # for t in reversed(range(lives)):'''
...
def show_gameover_screen():
    if game_over:
        ...
        draw_text(gameDisplay, "Press a key to begin!", 60, 430,370,BROWN)
...
#welcome_screen():
  ...
  #     gameDisplay.blit(background, (0, 0)) # display background screen on welcome
   # draw_text(gameDisplay, "FRUIT NINJA!", 90, WIDTH / 2, HEIGHT / 4)
  #  game_over1 = pygame.image.load("images/game_over.png")  # game background

   # background = pygame.transform.scale(background, (800, 500))
   # if not game_over:
       # game_over1 = pygame.image.load("images/game_over.png")  # game background
       # gameDisplay.blit(pygame.image.load("images/game_over.png"))
    #    draw_text(gameDisplay, "Your Total Score :: " + str(score), 50, WIDTH / 2, HEIGHT / 2) '''
